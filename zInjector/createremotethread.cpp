...
bool CreateRemoteThreadMethod( Process process, std::string dll_path )
{
	...
	LPVOID base_address = VirtualAllocEx( process.GetHandle( ), nullptr, dll_path.size( ) + 1, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE );
	if ( !base_address )
		return false;

	// Grab the LoadLibraryA address
	LPVOID load_library_address = GetProcAddress( GetModuleHandleA( "kernel32.dll" ), "LoadLibraryA" );
	if ( !load_library_address )
		return false;

	// Copy the module name/path to the previously allocated memory (required by LoadLibraryA)
	if ( !WriteProcessMemory( process.GetHandle( ), base_address, dll_path.c_str( ), dll_path.size( ) + 1, nullptr ) )
		return false;

	// Create a new thread in our target process pointing to our LoadLibraryA function
	if ( !CreateRemoteThread( process.GetHandle( ), nullptr, NULL, LPTHREAD_START_ROUTINE( load_library_address ), base_address, NULL, nullptr ) )
		return false;
