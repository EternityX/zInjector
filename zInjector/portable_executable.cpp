...
namespace wpm
{
	...
	HANDLE PortableExecutable::CreateMapView( )
	{
		TCHAR buffer[ MAX_PATH ];
		if ( !K32GetModuleFileNameExA( m_process->GetHandle( ), nullptr, buffer, MAX_PATH ) ) {
			return nullptr;
		}
	...
	PIMAGE_DOS_HEADER PortableExecutable::FetchDOSHeader( HANDLE map_view )
	{
	...
	PIMAGE_NT_HEADERS PortableExecutable::FetchImageHeader( )
	{
		auto dos_header = PortableExecutable::FetchDOSHeader( PortableExecutable::CreateMapView( ) );
		if ( !dos_header ) {
			return nullptr;
		}
	...
	{
		PIMAGE_NT_HEADERS nt_header = PortableExecutable::FetchImageHeader( );
		if ( !nt_header ) {
			return false;
		}
	...
	std::vector<PortableExecutable::IMPORT_INFO> PortableExecutable::FetchImports( )
	{
		std::vector<IMPORT_INFO> return_imports;

		HANDLE file = CreateFileA( m_process->FetchModuleFileName( ).c_str( ), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr );
		...
		auto nt_header = PortableExecutable::FetchImageHeader( );
		if ( !nt_header ) {
			return { };
		}
		...
		DWORD base = reinterpret_cast<DWORD>( PortableExecutable::CreateMapView( ) ) + section_header->PointerToRawData;
